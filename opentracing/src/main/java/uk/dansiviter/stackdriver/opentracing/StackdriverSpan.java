...
public class StackdriverSpan implements Span {
	...
	private final StackdriverSpanContext ctx;
	...
	private StackdriverSpan(StackdriverTracer tracer, Builder builder) {
		...
		final Optional<StackdriverSpanContext> parentCtx = parentContext();
		this.ctx = StackdriverSpanContext.builder(parentCtx, tracer.sampler()).build();
	}

	private Optional<StackdriverSpanContext> parentContext() {
		if (this.references.isEmpty()) {
			return Optional.empty();
		}

		Ref parent = references.get(0);
	...
	@Override
	public StackdriverSpanContext context() {
		return this.ctx;
	}
	 ...
	 * @return
	 */
	static Tracer.SpanBuilder builder(@Nonnull String operationName, @Nonnull StackdriverTracer tracer) {
	...
	private static class Builder implements Tracer.SpanBuilder {
		...
		private Builder(@Nonnull String operationName, @Nonnull StackdriverTracer tracer) {
			this.operationName = operationName;
			this.tracer = tracer;
		}
		...
		public SpanBuilder addReference(String referenceType, SpanContext referencedContext) {
			final Ref ref = new Ref(referenceType, (StackdriverSpanContext) referencedContext);
	...
	static class Ref {
		final String type;
		final StackdriverSpanContext ctx;
